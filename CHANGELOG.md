...
## [0.2.0] - 2026-09-04
### Added
- **100% Transparent Data AST**: Refactored all effect constructors in `com.lambdaseq.fx.core` into dedicated `defrecord` types implementing `ITagged` and `IEffect`.
...
- **Pipeline Utilities & Metaprogramming (`com.lambdaseq.fx.utils`)**: Added traversal, querying, AST rewriting, splicing, and combinator composition (`pipe>`, `comp>`, `pipe-fx-fn>`, `around>`, `with-scoped-service>`, `compose-ast-passes`).
