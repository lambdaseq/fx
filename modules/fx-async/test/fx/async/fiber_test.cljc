  ...
  (testing "interrupting a fiber transitions its status to :interrupted and runs finalizers"
    (let [started? (promise)
      ...
      (let [outcome (fxa/join-fiber! fib 1000 :timeout)]
        ...
        (is (= true #?(:clj (deref cleaned? 1000 nil)
                       :cljs @cleaned?)))))))
    ...
    (let [child-started? (promise)
          ...
          parent-fib (fxa/run-fiber!
                      (-> (fxa/fork> (fx/acquire-release>
                          ...
                          (fx/mapcat> (fn [_]
                                        (fx/sleep> 3000)))))]
      #?(:clj (deref child-started? 2000 nil)
         :cljs @child-started?)
      (let [child-fib #?(:clj (deref child-fib-ref 2000 nil)
                         :cljs @child-fib-ref)]
        (is (some? child-fib))
        ...
        (let [child-outcome (fxa/join-fiber! child-fib 2000 :timeout)]
          ...
          (is (= true #?(:clj (deref child-cleaned? 2000 nil)
                         :cljs @child-cleaned?))))))))
